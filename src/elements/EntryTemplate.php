...
class EntryTemplate extends Element
{
    ...
    return [
      [
        ...
        "orderBy" => function (int $dir) {
          if ($dir === SORT_ASC) {
            if (Craft::$app->getDb()->getIsMysql()) {
              return new Expression(
                "[[elements.dateUpdated]] IS NOT NULL DESC, [[elements.dateUpdated]] ASC"
              );
            } else {
          ...
          if (Craft::$app->getDb()->getIsMysql()) {
            return new Expression(
              "[[elements.dateUpdated]] IS NULL DESC, [[elements.dateUpdated]] DESC"
            );
          } else {
  ...
  protected static function defineFieldLayouts(?string $source): array
  {
    if ($source !== null) {
      if ($source === "*") {
        ...
        $sections = Craft::$app
          ->getEntries()
          ->getSectionsByType(Section::TYPE_SINGLE);
      } else {
        $sections = [];
      ...
      $entryTypes = array_values(
        array_unique(
          array_merge(
            ...array_map(
              fn(Section $section) => $section->getEntryTypes(),
              $sections
            )
          )
        )
   ...
   * @inheritdoc
   */
  public function prepareEditScreen(
    Response $response,
    string $containerId
  ): void {
  ...
  public function afterSave(bool $isNew): void
  {
    ...
    $this->previewImage = $request->getBodyParam("previewImage")
      ? $request->getBodyParam("previewImage")[0]
      : null;
      ...
      if (!$sectionIds && !$isNew) {
        ...
        if (!$record) {
          throw new InvalidConfigException(
            "Invalid entry templates ID: $this->id"
          );
        }
  ...
  public function getPreviewImageUrl(?array $transform = null): ?string
  {
    return $this->previewImage
      ? EntryTemplates::$plugin->previewImages->getPreviewImageUrl(
        $this->previewImage,
        $transform
      )
      : null;
  ...
  private function _can(string $action, User $user): bool
  {
    ...
    return parent::{$can}($user)
      ? true
      : $this->_mockEntryForPermissionChecks()->{$can}($user);
  }
   ...
   */
  private function _fieldLayoutWithoutEntryTitleField(
    ...
    $fields[] = Cp::textareaFieldHtml([
      ...
      "inputAttributes" => [
        "aria" => [
          "label" => Craft::t(
            "entrytemplates",
            "The description to use for this entry template."
          ),
        ],
      ],
    ]);
